...
internal class MtkPreloaderParserService
{
	...
	{
		int num = Extension.Find(data, new byte[8] { 77, 77, 77, 1, 56, 0, 0, 0 });
		if (num == -1)
		{
		...
		num = Extension.Find(data, Encoding.ASCII.GetBytes("MTK_BLOADER_INFO_v"));
		if (num == -1)
		{
		...
		checked
		{
			if (uint.TryParse(Encoding.ASCII.GetString(data, "MTK_BLOADER_INFO_v".Length, 2), out var result))
			{
				...
				if (array.Length - 4 > 4)
				{
					...
					if (!isXFlash)
					{
						int num4 = Extension.Find(array, Encoding.ASCII.GetBytes("MTK_BIN"));
						if (num4 != -1)
						{
	...
	{
		int num = Extension.Find(data, Encoding.ASCII.GetBytes("MTK_BLOADER_INFO"));
		byte[] bytes = data.Skip(checked(num + 27)).Take(48).ToArray();
